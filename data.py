...
class Data:
    ...
    def read(self, filepath):
        ...
        with open(filepath, newline ='') as file:
            ...
            curr_line = 0 # counter
            type = [] # variable for headers
            self.header2col = {} #create new dictionary
            self.data = [] #create new nd array
            self.headers = [] #create new list of strings
            
            #this for loop goes through the lines in the csv file
            for row in csvreader:
                if curr_line == 0: #the first line is the header
                    headers = [] 
                    for item in row:
                        headers.append(item.strip()) #add to list and remove white space before and after
                    curr_line += 1 #increment
                elif curr_line==1: #row after the header
                    if 'numeric' not in row:
                        raise Exception('No numeric data found in the row below the headers. TO FIX: add row containing data types below the headers')
                    index = 0
                    for val in row:
                        type.append(val.strip()) #add each val in the row 
                        if val.strip() == 'numeric': #type = numeric
                            self.headers.append(headers[index]) #add header
                           # self.header2col[headers[index]] = index
                        index += 1 #increment
                    curr_line += 1 #increment
                else:
                    curr_row = [] #new list
                    index = 0
                    for item in row:
                        if type[index] == 'numeric': # if the current is numeric
                            curr_row.append(item) # add it to be converted to an array
                        index += 1 #increment
                    self.data.append(curr_row) #add current row to data
            self.data = np.array(self.data, np.single) # convert to an np array
            for col_index in range(self.data.shape[1]): # make header2 col dictionary
    ...
    def __str__(self):
        ...
        final_str = str(self.filepath) + '(' + str(self.data.shape[0]) + 'x' + str(self.data.shape[1]) + ')' + '\n'
        final_str += 'Headers: \n'
        ...
        if self.data.shape[0] > 5: # if there are more than 5 rows
            final_str += 'Showing first 5/' + str(self.data.shape[0]) + 'rows \n'
            for i in range(5):
            ...
            for i in range(self.data.shape[0]):
                final_str += "Showing all " + str(self.data.shape[0]) + ' rows'
                for val in self.data[i]:
                    final_str += str(val) + '\t'
                final_str += '\n'
        return final_str
    ...
    def get_header_indices(self, headers):
        ...
        nn_ints = []
        for val in headers:
            nn_ints.append(self.header2col[val])
        return nn_ints
